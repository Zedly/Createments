...
public class WatcherArrow implements Listener {
    ...
    public boolean shoot(EntityShootBowEvent evt) {
        if (evt.getEntity() instanceof Player) {
            ...
            if (arrowShot != null && arrowShot.getItemMeta().hasLore()) {
                ...
                if (ElementalArrow.arrows.containsKey(type) && player.hasPermission("zenchantments.arrow.use") && !evt.getBow().getEnchantments().containsKey(Enchantment.ARROW_INFINITE)) {
                    ElementalArrow arrow = (ElementalArrow) Utilities.construct(ElementalArrow.arrows.get(type).getClass(), (Projectile) evt.getProjectile());
                    Set<AdvancedArrow> a = new HashSet<>();
                    a.add(arrow);
                    Storage.advancedArrows.put(evt.getProjectile(), a);
    ...
    public boolean impact(ProjectileHitEvent evt) {
        if (Storage.advancedArrows.containsKey(evt.getEntity())) {
            Set<AdvancedArrow> ar = (Set<AdvancedArrow>) Storage.advancedArrows.get(evt.getEntity());
    ...
    public boolean entityHit(EntityDamageByEntityEvent evt) {
        if (evt.getDamager() instanceof Arrow) {
            if (Storage.advancedArrows.containsKey(evt.getDamager())) {
                Set<AdvancedArrow> arrows = (Set<AdvancedArrow>) Storage.advancedArrows.get(evt.getDamager());
                for (AdvancedArrow arrow : arrows) {
                    ...
                    Storage.advancedArrows.remove(evt.getDamager());
