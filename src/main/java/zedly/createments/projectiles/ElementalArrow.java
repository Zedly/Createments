...
import zedly.createments.Utilities;
...
public class ElementalArrow implements AdvancedArrow {
    ...
    public static Map<String, ElementalArrow> arrows = new HashMap<>();
    ...
    public void die() {
        ...
        Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Storage.createments, () -> {
            if (Storage.advancedArrows.containsKey(e)) {
                if (Storage.advancedArrows.get(e).size() == 1) {
                    Storage.advancedArrows.remove(e);
                } else {
                    Storage.advancedArrows.get(e).remove(arrow2);
                }
            }
        }, 1);
    }
...
//Elemental Arrows
    public static class ArrowBouncifying extends ElementalArrow {
        ...
        public void onImpact() {
            ...
            for (Entity ent : getArrow().getNearbyEntities(4, 3, 4)) {
                if (Utilities.attackEntity((LivingEntity) ent, (Player) getArrow().getShooter(), 0)) {
    ...
    public static class ArrowGrenade extends ElementalArrow {
        ...
        public void onImpact() {
            for (int i = 0; i < balls; i++) {
                ...
                a.add(ar);
                Storage.advancedArrows.put(ball, a);
            }
            die();
        }
    ...
    public static class ArrowHarpoon extends ElementalArrow {
        ...
        public boolean onImpact(EntityDamageByEntityEvent evt) {
            ...
            if (humans || !(ent instanceof Player)) {
                if (Utilities.attackEntity((LivingEntity) evt.getEntity(), (Player) shooter, 0)) {
    ...
    public static class ArrowLightning extends ElementalArrow {
        ...
        public void onImpact() {
            ...
            if (!unsafe) {
                Bukkit.getScheduler().scheduleSyncDelayedTask(Storage.createments, () -> {
